      ...
      in case redeemer of
        ...
        Transform -> 
          case getOutboundDatum contTxOutputs of
            ...
            (Offering ptd' _ _) -> (signedBy txSigners walletPkh)                         -- seller must sign it
                                && (isNInputs txInputs 1 && isNOutputs contTxOutputs 1)   -- single tx going in, single going out
                                && (ptd == ptd')                                          -- seller cant change
                                && (isTxOutsideInterval lockTimeInterval txValidityRange) -- seller can unlock it
            ...
            (Auctioning ptd' atd td') -> (signedBy txSigners walletPkh)                         -- seller must sign it
                                      ...
                                      && (checkValidTimeLock td td')                            -- valid global time lock
                                      && (isTxOutsideInterval lockTimeInterval txValidityRange) -- seller can unlock it
            
            -- -- transform a swappable state into a bid
            (Bidding ptd' _) -> (signedBy txSigners walletPkh)                         -- seller must sign it
                             && (isNInputs txInputs 1 && isNOutputs contTxOutputs 1)   -- single tx going in, single going out
                             && (ptd == ptd')                                          -- seller cant change
                             && (isTxOutsideInterval lockTimeInterval txValidityRange) -- seller can unlock it
        ...
        (Update aid) -> let !incomingValue = validatingValue + adaValue (adaInc aid) in
          case getOutboundDatumByValue contTxOutputs incomingValue of
            ...
            (Auctioning ptd' atd td') -> (signedBy txSigners walletPkh)                       -- seller must sign it
                                      && ((ptd == ptd') && (td == td'))                       -- seller and time can't change
                                      && (isNInputs txInputs 1 && isNOutputs contTxOutputs 1) -- single tx going in, single going out
                                      && (checkValidTimeData atd)                             -- valid time lock
        ...
        (FRRemove ptd' st) -> let !buyerPkh  = ptPkh ptd'
                                  ...
                                  !thisTkn   = getTokenName pd st in
                              (signedBy txSigners buyerPkh)                                                -- buyer must sign
        ...
        (ORemove mod) -> let !txId = createTxOutRef (moTx mod) (moIdx mod)
          in case getDatumByTxId txId of
            -- offering only
            (Offering ptd' _ ofd) -> let !buyerAddr = createAddress (ptPkh ptd') (ptSc ptd') in
                                     (signedBy txSigners walletPkh)                                 -- seller must sign it
                                  && (isAddrGettingPaidExactly txOutputs buyerAddr validatingValue) -- buyer must be paid
      ...
      in case redeemer of
        ...
        Transform -> 
          case getOutboundDatum contTxOutputs of
            ...
            (Swappable ptd' _ td') -> traceIfFalse "Incorrect Tx Signer" (signedBy txSigners walletPkh)                         -- seller must sign it
                                   && traceIfFalse "Too Many In/Out"     (isNInputs txInputs 1 && isNOutputs contTxOutputs 1) -- single tx going in, single going out
                                   && traceIfFalse "Datum Is Changing"   (ptd == ptd')                                        -- seller cant change
                                   && traceIfFalse "Invalid Time Change" (checkValidTimeData td')                             -- valid time lock
