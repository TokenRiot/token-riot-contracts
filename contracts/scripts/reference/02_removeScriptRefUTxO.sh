...
payee_pkh=$(${cli} address key-hash --payment-verification-key-file ../wallets/profit-wallet/payment.vkey)
...
payee_address_out="${payee_address} + ${min_utxo} + ${asset}"
echo "Exit OUTPUT: "${payee_address_out}
#
# exit
#
# get payee utxo
...
alltxin=""
TXIN=$(jq -r --arg alltxin "" --arg payeePkh "${payee_pkh}" 'to_entries[] | select(.value.inlineDatum.fields[4].bytes == $payeePkh) | .key | . + $alltxin + " --tx-in"' ../tmp/script_utxo.json)
script_tx_in=${TXIN::-8}
...
FEE=$(${cli} transaction build \
    ...
    --spending-reference-tx-in-redeemer-file ../data/reference/manage-redeemer.json \
    --tx-out="${payee_address_out}" \
    --required-signer-hash ${payee_pkh} \
